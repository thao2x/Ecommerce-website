...
export default {
    ...
    computed: {
        text() {
            return this.type == 'signin' ? 'Sign Up' : 'Sign In';
        },
        description() {
            return this.type == 'signin' ? 'Don\'t have an account ?' : 'Already have an account ?';
        }
    },
    methods: {
        goToPage: function () {
            const routerName = this.type == 'signin' ? 'sign-up' : 'sign-in';
